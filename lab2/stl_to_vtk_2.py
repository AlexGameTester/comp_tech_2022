...
class CalcMesh:
    ...
    def snapshot(self, snap_number):
        ...
        acc = vtk.vtkDoubleArray()
        acc.SetName("Oz acceleration")
        ...
        for i in range(0, len(self.nodes[0])):
            ...
            acc.InsertNextValue(self.acceleration[i])
        ...
        unstructuredGrid.GetPointData().AddArray(acc)
...
try:
    ...
    gmsh.merge(os.path.join(path, 'Honeycomb_organizer.stl'))
except:
